...
describe('PanelBridge.lua handlers.generateWeather', () => {
  it('warm front (frontType 2): uses triggerCustomWeather', () => {
    const bridge = loadPanelBridge(LUA_PATH, climateStub({ periodAlreadyRunning: false }));
  ...
  it('stationary front fails when the Build 42 API cannot represent it', () => {
    const bridge = loadPanelBridge(LUA_PATH, climateStub({ periodAlreadyRunning: false }));
    const result = bridge.callHandler('generateWeather', { strength: 0.7, frontType: 0 });

    expect(result.ok).toBe(false);
    expect(result.err).toContain('No generate weather method available');
  });
});
