...
// apps/panel-server/utils/errorCodes.js: 8 pre-i18n error codes ship a frozen
...
// already renders the matrix from (apps/panel-client/src/locales/*/roles.json). Not
...
export function getRecoveryUrl(error: unknown): string | null {
  ...
  // from the SAME failed RCON handshake (apps/panel-server/routes/rcon.js POST
  // /connect and /test, and apps/panel-server/routes/config.js POST /test-rcon) and
  ...
  // EACCES/permission-denied: apps/panel-server/routes/server.js's formatDirectoryReadError
