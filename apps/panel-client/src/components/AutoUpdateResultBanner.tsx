...
export function AutoUpdateResultBanner() {
  ...
  useEffect(() => {
    ...
    updateApi
      .getStatus()
      .then((status) => {
        if (!cancelled) setResult(status.lastAutoUpdateResult)
      })
      .catch(() => {
        /* keep no-banner state; this is best-effort */
      })
    return () => {
      cancelled = true
    }
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 space-y-0.5">
        ...
        >
          {isFailure ? 'Automatic update failed' : 'Automatic update completed'}
        </p>
        {isFailure && (
          <p className="text-sm font-medium text-foreground">
            {getAutoUpdateServerStateMessage(result.serverUp)}
          </p>
        )}
        <p className="text-sm text-muted-foreground">
          {isFailure
            ? getAutoUpdateReasonMessage(result)
            : getAutoUpdateSuccessMessage(result)}
        </p>
      </div>
      <button
        type="button"
