...
export function MountDiscoveryBanner({
  mount,
  onConnect,
}: MountDiscoveryBannerProps) {
    ...
    <div className="flex flex-wrap items-center gap-3 rounded-md border bg-muted/10 px-3 py-2">
      ...
      <div className="flex shrink-0 items-center gap-1">
        <Button
          ...
          className="h-8 w-8"
          onClick={dismiss}
          aria-label={'Dismiss discovered install'}
          title={'Dismiss'}
        >
          <X className="h-4 w-4" />
        </Button>
        <Button variant="outline" size="sm" onClick={() => onConnect(mount)}>
          <Plus className="me-1.5 h-3.5 w-3.5" aria-hidden="true" /> {'Add'}
        </Button>
      </div>
    </div>
  )
