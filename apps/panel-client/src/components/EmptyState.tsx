...
export function EmptyState({
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center ${padding} px-4 text-center ${className}`}
      aria-live="polite"
      aria-atomic="true"
    >
      <div className="relative mb-4">
        <div
          className={`${containerSize} empty-state-aura rounded-2xl border border-border/50 bg-muted/50 flex items-center justify-center empty-state-icon`}
          aria-hidden="true"
        >
          {icon || (
            <IconComponent className={`${iconSize} text-muted-foreground/40`} />
          )}
        </div>
      </div>
      <p className="mb-2 text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground/80">
        {eyebrow}
      </p>
      <h3
        className={`${compact ? 'text-base' : 'text-lg'} font-semibold text-foreground/80 mb-1`}
      >
        {title}
      </h3>
      {description && (
        <p
          className={`${compact ? 'text-xs' : 'text-sm'} max-w-sm leading-6 text-muted-foreground`}
        >
          {description}
        </p>
      )}
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          {secondaryAction && (
            <EmptyStateActionButton
              action={{ variant: 'ghost', ...secondaryAction }}
              compact={compact}
            />
          )}
        </div>
