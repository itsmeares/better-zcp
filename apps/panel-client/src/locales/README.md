...
1. **Register it.** Add one row to `LANGUAGES` in `apps/panel-client/src/i18n/languages.ts`:
   ```ts
...
2. **Add the folder.** Create `apps/panel-client/src/locales/de/` and copy every `.json` file from `apps/panel-client/src/locales/en/` into it (same filenames — each one is a namespace, e.g. `login.json`, `setup.json`, `shell.json`). Translate the values; never rename or add keys, only translate them.
